...
- With your Mattermost server already running, run `ngrok http 8065` to make your Mattermost server available for webhook requests.
...
1. The plugin health check job needs to be disabled. This can be done by setting the server config setting `PluginSettings.EnableHealthCheck` to `false`. Note that if your plugin crashes, you will need to restart it, using `make reset` for example. This command will also kill any currently running `delve` process. If you want to continue debugging with `delve`, you will need to run `make debug-plugin` again after restarting the plugin.
