...
Unit tests are usually written for parts of the `common, main,` and `app` modules, and usually cover individual functions or classes.
...
In order to ensure that most of the app is covered, we try to maintain 70% coverage of the `common, main,` and `app` modules.
You can view a coverage map by running this command: `npm run test:unit-coverage`. 
...
To build the app and run the E2E tests from the root directory, you can run the following command: `npm run e2e`. This command builds the app for testing, installs E2E dependencies, and then runs the E2E tests.

You can also run subsets of the tests by filtering using `grep` when running the E2E tests from within the `e2e` directory. 
