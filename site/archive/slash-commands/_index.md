...
Slash commands are messages that begin with `/` to trigger an HTTP request to a web service that can, in turn, post one or more messages in response.

Unlike outgoing webhooks, slash commands work in private channels, direct messages, and group messages, in addition to public channels, and can be configured to auto-complete when typing. Mattermost includes a number of built-in slash commands. This document focuses on the slash commands that can be configured as custom integrations.


## Basic usage

Follow [the custom slash command](https://docs.mattermost.com/developer/slash-commands.html#custom-slash-command) documentation to create a slash command.
...
If you configured a slash command with a trigger word of `test`, a request URL of `http://example.com/slash-command`, and a `POST` request method, and someone posts the following message in the Town Square channel:
...
Upon receiving one of these requests, your integration should immediately confirm that the `token` provided in the request body matches the one given to you after the slash command was created. If it doesn't match, we strongly recommend that you reject the request. This ensures that the request actually came from Mattermost.

Once the request is verified, you can send a message in the channel where the slash command was invoked by responding with a 200 status code and including a response body as follows:
...
Any requests made to the response URL should either be a plain text or JSON-encoded body. The JSON-encoded message supports both [Markdown formatting](https://docs.mattermost.com/messaging/formatting-text.html) and [message attachments](https://docs.mattermost.com/developer/message-attachments.html).
...
During development, you may override this behaviour by setting `ServiceSettings.AllowedUntrustedInternalConnections` to `"127.0.0.0/8"` in your `config.json` or via System Console **Advanced > Developer**. See the [Mattermost configuration settings](https://docs.mattermost.com/configure/configuration-settings.html#allow-untrusted-internal-connections-to) documentation for more details.
...
See the [Slack compatibility](https://docs.mattermost.com/developer/slash-commands.html#slack-compatibility) documentation for details.
